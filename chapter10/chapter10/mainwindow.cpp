...
void MainWindow::on_NEmbed_triggered()
{
    //非盲水印，最低有效位，嵌入过程
    //读取载体图像
    QString filename = QFileDialog::getOpenFileName(this,tr("打开载体图像"),"",tr("Image File(*.bmp *.jpg *.jpeg *.png)"));
    QTextCodec *code = QTextCodec::codecForName("gb18030");
    std::string name = code->fromUnicode(filename).data();
    srcImage = cv::imread(name);
    if(!srcImage.data)
    ...
    //只能嵌入二进制值图像，如果是彩色图像需要先转换成灰度图像再处理为二进制值图像
    //cvtColor( wmImage,wmImage, COLOR_RGB2GRAY );
    Mat wmImageShow;  //该Mat专门用来显示用。
    // threshold(wmImage,wmImageShow, 127, 255, THRESH_BINARY );
    wmImage.copyTo(wmImageShow);
    //阈值处理，通常水印图像中只有0和255两种值，将其中的255处理为1.
    threshold(wmImage,wmImage, 127, 1, THRESH_BINARY );   //保留最低位有效位LSB即可。
    //提取矩阵，用于提取最低有效位
    Mat t(MM,NN,CV_8UC1,1);
    t.copyTo(embedSrc);
    //通过按位与运算，将bgr[0]的最低有效位提取出来
    cv::bitwise_and(bgr[0],t,embedSrc);
    //通过按位异或操作，将原始水印与得到载体图像R通道的最低有效位进行运算，得到嵌入信息embedSrc.
    ...
    //通过将载体图像的高7位，与得到的只有一位的嵌入信息进行按位或，完成将嵌入信息embedSrc嵌入到载体图像的R通道内。
    cv::bitwise_or(t3,embedSrc,embedSrc);
...
void MainWindow::on_NExtract_triggered()
{
    ...
    //*****************************提取含水印图像的R通道最低有效位****************************************************
    //读取含水印图像
    filename = QFileDialog::getOpenFileName(this,tr("打开含水印载体图像"),"",tr("Image File(*.bmp *.jpg *.jpeg *.png)"));
    name = code->fromUnicode(filename).data();
    embedSrc = cv::imread(name,IMREAD_UNCHANGED);
    if(!embedSrc.data)
    {
        QMessageBox msgBox;
        msgBox.setText(tr("未找到数据"));
...
void MainWindow::on_NBRembed_triggered()
{
    ...
    */
    ...
    //嵌入水印
    //将特定位与水印信息进行比较。
    //嵌入水印是0时，不需要操作即完成水印的嵌入。
    //嵌入水印是1时，需要判断嵌入位的值决定如何运算。
    int flag=0;
    int n;
    for(i=0;i<M;i++)
        for(j=0;j<N;j++)
